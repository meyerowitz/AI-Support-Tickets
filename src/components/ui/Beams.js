...
const CanvasWrapper = ({ children }) => (
  <Canvas dpr={[1, 2]} frameloop="always" gl={{ alpha: true }} className="beams-container" >
    {children}
  </Canvas>
);
...
const Beams = ({
  ...
  vec3 getNormal(vec3 pos) {
    ...
    gl_FragColor.rgb -= randomNoise / 15. * uNoiseIntensity;`
        ...
        material: { fog: true },
        uniforms: {
          diffuse: new THREE.Color(...hexToNormalizedRGB('#020822')),
...
const MergedPlanes = forwardRef(({ material, width, count, height }, ref) => {
  ...
  const geometry = useMemo(
    () => createStackedPlanesBufferGeometry(count, width, height, 0, 5),
    [count, width, height]
  );
  useFrame((_, delta) => {
